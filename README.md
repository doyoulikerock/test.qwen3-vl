...
| `--json` | off | 답변 대신 JSON(구간·샘플링 시각·질문·답변·단계별 시간) 출력 |
...
* **ask 모드** — 질문과 (선택적) `start`/`end` 타임코드를 넣으면 답변이 위에 나오고, 그 답변의 **근거로 모델에 실제로 들어간 프레임들이 아래 그리드에 그대로** 표시된다. 범위를 비우면 영상 전체가 대상이다. 타임코드는 CLI와 같은 형식(`190`, `00:03:10`, `3:10`)을 받는다.
